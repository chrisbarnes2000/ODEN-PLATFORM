...
interface NodesTabProps {
  ...
  onClearEditing?: () => void;
  onOpenOriginalDoc?: (docId: string) => void;
}

export default function NodesTab({ project, setProject, initialEditingNodeId, onClearEditing, onOpenOriginalDoc }: NodesTabProps) {
  const [editingNodeId, setEditingNodeId] = useState<string | null>(null);
  const [isSidebarExpanded, setIsSidebarExpanded] = useState(true);
  const [mobileView, setMobileView] = useState<'nodes' | 'edges'>('nodes');
  ...
  const addNode = () => {
    ...
    setNewNode({ label: '', type: 'actor', description: '', tags: [], sources: [], sourceIds: [], placeholder: false, sectionId: '', phaseId: '' });
    setSourceInput('');
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {/* Main Event anchor */}
        <div className="mb-5 p-3 rounded-lg border border-border bg-bg/40">
          <div className="text-[10px] uppercase tracking-[2px] text-muted font-bold mb-2">Main Event Anchor</div>
          <div className="text-[12px] text-text/90 mb-2">
            Optional, but recommended. This sets a central reference point so your graph has a "home".
          </div>
          <div className="flex gap-2 items-center">
            ...
            >
              ...
              {project.nodes
                .filter(n => {
                  // Some imports / older saves may contain uppercase types (e.g. "CASE").
                  const t = String((n as any).type ?? '').toLowerCase();
                  return t === 'event' || t === 'case';
                })
                .map(n => (
            ...
            <button
              ...
              className="px-3 py-2 text-[10px] uppercase tracking-widest font-bold border border-border bg-panel hover:bg-white/5"
              title="Clear main event"
            >
              Clear
            </button>
          </div>
          <div className="text-[11px] text-muted mt-2">
            Tip: set this to your core claim/event, then use connections to branch outward.
          </div>
        </div>
        ...
        <div className="frow">
          ...
          <div className="text-[11px] text-muted leading-relaxed mb-2">
            Auto-filled by Smart Import. Use this to jump back to the originating text.
          </div>
          <div className="flex flex-wrap gap-2">
            {((newNode as any).documentIds || []).length === 0 && (
              <div className="text-[11px] text-muted">—</div>
            )}
            {((newNode as any).documentIds || []).map((docId: string) => (
              <button
                key={docId}
                type="button"
                className="btn btn-m flex items-center gap-2"
                onClick={() => onOpenOriginalDoc?.(docId)}
                title="Open the originating document and view the original import text"
              >
        ...
        <div className="frow">
          <label className="flex items-center gap-2">
            <LinkIcon size={14} />
            Attach from registry (recommended)
          </label>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                const src = project.sources.find(s => s.id === attachSourceId);
                if (!src) return;

                // Attach registry id
                setNewNode(prev => {
                  const ids = new Set(prev.sourceIds ?? []);
                  ids.add(src.id);

                  // Also mirror into legacy text sources list so other UI stays consistent
                  const lines = new Set(
                    sourceInput
                      .split('\n')
                      .map(l => l.trim())
                  ...
                  lines.add(src.url ? `${src.title} | ${src.url}` : `${src.title}`);
                  setSourceInput(Array.from(lines).join('\n'));

                  return { ...prev, sourceIds: Array.from(ids) };
                });
          ...
          {((newNode.sourceIds ?? []).length > 0) && (
            <div className="mt-2 space-y-1">
              {(newNode.sourceIds ?? []).map(id => {
                const s = project.sources.find(x => x.id === id);
                if (!s) return null;
                return (
                  <div key={id} className="flex items-center justify-between gap-2 text-[12px]">
                    <div className="min-w-0">
                      <span className="text-accent font-bold truncate">{s.title}</span>
                      <span className="text-muted"> {s.date ? `• ${s.date}` : ''}</span>
                    </div>
                    <button
                      type="button"
                      ...
                      onClick={() => {
                        setNewNode(prev => ({
                          ...prev,
                          sourceIds: (prev.sourceIds ?? []).filter(x => x !== id),
                        }));
                        // Also remove from textarea if it matches the registry entry line
                        const lineA = s.url ? `${s.title} | ${s.url}` : `${s.title}`;
                        const nextLines = sourceInput
                          .split('\n')
                          .map(l => l.trim())
                          .filter(Boolean)
                          .filter(l => l !== lineA);
                        setSourceInput(nextLines.join('\n'));
                      }}
                    >
                      <X size={14} />
                    </button>
        ...
        <div className="sec-hdr mt-8 mb-4 border-b border-border pb-2 text-muted uppercase text-[11px] tracking-[1px]">
